...
const FIREBASE_ERROR_MESSAGES: Record<string, string> = {
  ...
  "auth/invalid-uid":
    "User ID must be a non-empty string with at most 128 characters.",
  "auth/invalid-claims": "Custom user claims are invalid.",
  "auth/claims-too-large":
    "Custom user claims exceed the maximum allowed size.",
  ...
  "auth/invalid-creation-time":
    "Creation time must be a valid UTC date string.",
  "auth/invalid-disabled-field": "Disabled field must be a boolean value.",
  "auth/invalid-email-verified":
    "Email verified field must be a boolean value.",
  ...
  "auth/invalid-last-sign-in-time":
    "Last sign-in time must be a valid UTC date string.",
  ...
  "validation/username-invalid-chars":
...
export function parseFirebaseError(error: unknown): {
  ...
  if (error && typeof error === "object") {
    ...
    if (
      errorObj.message &&
      String(errorObj.message).includes("Network Error")
    ) {
      return {
...
export function getErrorMessage(errorCode: string): string {
  return (
    FIREBASE_ERROR_MESSAGES[errorCode] || FIREBASE_ERROR_MESSAGES["unknown"]
  );
}
...
export const validationRules = {
  ...
  username: (value: unknown) => {
    ...
    return !usernameRegex.test(strValue)
      ? "Username can only contain letters, numbers, and underscores"
      : null;
  },
};
